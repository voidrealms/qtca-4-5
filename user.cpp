...
void User::load(int value)
{
    ...
    QSqlQuery query;
    QString cmd = "SELECT id,age,name FROM voidrealms.users where id = :id;";
    query.prepare(cmd);
...
{
    QSqlQuery query;
    QString cmd = "DELETE FROM voidrealms.users where id = :id";
    query.prepare(cmd);
...
bool User::update()
{
    qInfo() << "Inserting row";
    QSqlQuery query;
    QString cmd = "UPDATE `voidrealms`.`users` SET `id` = :id, `name` = :name, `age` = :age WHERE `id` = :id;";
    query.prepare(cmd);
    query.bindValue(":id", id);
    query.bindValue(":name", name);
    query.bindValue(":age", age);
